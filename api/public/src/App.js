...
const DEMO_RESULT = {
  dish: "Bowl poulet grillé · Quinoa · Avocat",
  items: ["poulet grillé", "quinoa", "avocat", "tomates cerises", "citron"],
  calories: 520, protein: 42, carbs: 38, fat: 18,
  confidence: "high", confidence_reason: "Plat clairement visible",
  portion_note: "Environ 350g — bol normal"
};
...
function Avatar({ u, size = 36 }) {
  ...
  const bg = u?.plan === "premium" ? "#7F77DD22" : "#E1F5EE";
  const color = u?.plan === "premium" ? PURPLE : GREEN;
...
function Spinner() {
  return <div className="spin" style={{ width: 28, height: 28, border: `3px solid ${GREEN}33`, borderTopColor: GREEN, borderRadius: "50%", margin: "0 auto" }} />;
}
function ErrBox({ msg }) {
  return msg ? <div style={{ background: "#FFF1F1", border: `1px solid ${RED}44`, borderRadius: 10, padding: 10, fontSize: 13, color: RED, marginBottom: 10 }}>{msg}</div> : null;
}
...
function LoginScreen({ onLogin, onGoRegister }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onLogin({ ...profile, email: data.user.email, token: data.session.access_token, uid: data.user.id });
...
function RegisterScreen({ onRegister, onGoLogin }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!uid) throw new Error("Vérifie ta boîte mail pour confirmer ton compte.");
      await supabase.from("profiles").insert({ id: uid, first_name: f.firstName, last_name: f.lastName, plan: f.plan, scans_today: 0, total_scans: 0 });
      onRegister(f.email);
    } catch (e) { setErr(e.message); }
    setLoading(false);
  };
...
function ScannerTab({ user, onScanComplete, onUpgrade }) {
  ...
  const handleFile = f => {
    ...
    r.onload = e => { setImgSrc(e.target.result); setImgBase64(e.target.result.split(",")[1]); setImgType(f.type || "image/jpeg"); setState("idle"); setResult(null); setAdded(false); setErr(null); };
    r.readAsDataURL(f);
  };
...
const res = await fetch("/api/analyze", {
  ...
  body: JSON.stringify({
    ...
    messages: [{
      ...
      if (parsed.error) { setErr(parsed.error); setState("error"); } else { setResult(parsed); setState("result"); }
    } catch (e) { setErr("Erreur analyse. Essaie le mode Démo."); setState("error"); }
  }, [imgBase64, imgType, canScan]);

  const addToJournal = async () => {
    if (!result || added) return;
    try {
      await supabase.from("scans").insert({ user_id: user.uid, dish: result.dish, calories: result.calories, protein: result.protein, carbs: result.carbs, fat: result.fat, confidence: result.confidence });
      const newToday = scansToday + 1;
      await supabase.from("profiles").update({ scans_today: newToday, total_scans: (user.total_scans || 0) + 1, last_scan_date: today }).eq("id", user.uid);
      onScanComplete({ scans_today: newToday, total_scans: (user.total_scans || 0) + 1, last_scan_date: today });
      setAdded(true);
    } catch (e) { setErr("Erreur enregistrement."); }
  };
  ...
  return <div style={{ padding: "16px 16px 20px" }}>
    ...
    {user.plan === "premium" && <div style={{ background: "#7F77DD11", border: `1px solid ${PURPLE}33`, borderRadius: 12, padding: "8px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}><span>✦</span><span style={{ fontSize: 12, color: PURPLE, fontWeight: 600 }}>Scans illimités · Premium</span></div>}
    ...
    {state === "loading" && <div className="pulse" style={{ textAlign: "center", padding: 20 }}><Spinner /><div style={{ color: GREEN, fontWeight: 600, fontSize: 13, marginTop: 10 }}>L'IA analyse ton assiette...</div></div>}
...
function ProfileTab({ user, onLogout, onUpgrade, onProfileUpdate }) {
  ...
  const upgradePlan = async () => {
    await supabase.from("profiles").update({ plan: "premium" }).eq("id", user.uid);
...
function Backoffice({ onClose }) {
  ...
  const load = async () => {
    ...
    const [{ data: u }, { data: s }] = await Promise.all([
      ...
      supabase.from("scans").select("*").order("scanned_at", { ascending: false }).limit(100)
    ]);
    setUsers(u || []); setScans(s || []); setLoading(false);
  };
  useEffect(() => { load(); }, []);

  const togglePlan = async (uid, cur) => {
    const np = cur === "premium" ? "free" : "premium";
    await supabase.from("profiles").update({ plan: np }).eq("id", uid);
  ...
  const StatCard = ({ label, value, sub, color = GREEN }) => (
    <div style={{ background: "#1E1E3F", borderRadius: 12, border: "1px solid #2A2A5A", padding: 14, textAlign: "center" }}>
  ...
  return <div style={{ background: "#12122A", minHeight: "100vh" }}>
    <div style={{ background: "#1A1A3E", padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #2A2A5A" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={load} style={{ color: "#aaa", background: "transparent", border: "1px solid #3A3A6A", borderRadius: 8, padding: "5px 10px", fontSize: 12 }}>↻</button>
    ...
    <div style={{ padding: 14 }}>
      ...
      {!loading && boTab === "dashboard" && <div className="fi">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          ...
          <StatCard label="Utilisateurs Free" value={free} color={ORANGE} />
        </div>
        ...
        {filtered.map(u => (
          <div key={u.id} style={{ background: "#1A1A3E", borderRadius: 12, border: "1px solid #2A2A5A", padding: 12, marginBottom: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <Avatar u={u} size={34} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#ddd" }}>{u.first_name} {u.last_name}</div>
                <div style={{ fontSize: 11, color: "#666" }}>{u.total_scans || 0} scans · depuis {u.created_at?.slice(0, 10)}</div>
              </div>
              <Badge plan={u.plan} />
            </div>
        ...
        {scans.slice(0, 30).map(s => {
          ...
          return <div key={s.id} style={{ background: "#1A1A3E", borderRadius: 12, border: "1px solid #2A2A5A", padding: "10px 12px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: "#666" }}>{u ? `${u.first_name} ${u.last_name}` : "—"} · {s.scanned_at?.slice(0, 10)}</div>
            </div>
            <div style={{ textAlign: "right" }}>
...
export default function App() {
  ...
  const logout = async () => { await supabase.auth.signOut(); setUser(null); setScreen("login"); setTab("scanner"); };
  const onScanComplete = updates => setUser(p => ({ ...p, ...updates }));
  const onProfileUpdate = updates => setUser(p => ({ ...p, ...updates }));
  const upgradePremium = async () => { await supabase.from("profiles").update({ plan: "premium" }).eq("id", user.uid); setUser(p => ({ ...p, plan: "premium" })); setShowUpgrade(false); };

  if (showBO) return <><style>{globalCss}</style><Backoffice onClose={() => setShowBO(false)} /></>;

  const wrap = children => <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: "#fff" }}><style>{globalCss}</style>{children}</div>;

  if (screen === "login") return wrap(<><div style={{ padding: "32px 0 8px", textAlign: "center" }}><div style={{ display: "inline-block", width: 56, height: 56, background: `${GREEN}15`, borderRadius: 18, lineHeight: "56px", fontSize: 28 }}>🥗</div></div><LoginScreen onLogin={login} onGoRegister={() => setScreen("register")} /></>);
  if (screen === "register") return wrap(<RegisterScreen onRegister={register} onGoLogin={() => setScreen("login")} />);
  if (screen === "confirm") return wrap(<ConfirmScreen email={confirmEmail} onGoLogin={() => setScreen("login")} />);
